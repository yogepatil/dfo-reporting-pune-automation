...
- TEST for the public API (only tests with sqlite3)
    - `_TestSQLApi` base class
    - `TestSQLApi`: test the public API with sqlalchemy engine
    - `TestSQLiteFallbackApi`: test the public API with a sqlite DBAPI
      connection
- TEST for the different SQL flavors (flavor specific type conversions)
    - TEST for the sqlalchemy mode: `_TestSQLAlchemy` is the base class with
      common methods, `_TestSQLAlchemyConn` tests the API with a SQLAlchemy
      Connection object. The different tested flavors (sqlite3, MySQL,
      PostgreSQL) derive from the base class
    - TEST for the fallback mode (`TestSQLiteFallback`)
...
class TestSQLApi(SQLAlchemyMixIn, _TestSQLApi):
    ...
    TEST for `read_sql_table` are included here, as this is specific for the
    sqlalchemy mode.
...
class _TestSQLAlchemy(SQLAlchemyMixIn, PandasSQLTest):
    ...
    Subclasses for specific database types are created below. TEST that

WITH src
     AS (SELECT DISTINCT(VISIT_EVENT_ID) AS VIS_ID,
                ...
                parse_json(VISIT_CUSTOM_VARIABLES):workflowId::text as workflowId
...
SELECT VIS_ID as src_visitEventid,
       ...
       substr(s.workflowId,1,36) AS src_workflowId,
       t.PROACTIVE_WORKFLOW_ID AS trg_proactiveWorkflid,
       CASE
         WHEN src_workflowId IS NULL AND trg_proactiveWorkflid IS NULL THEN true
         WHEN src_workflowId = trg_proactiveWorkflid THEN true
         ELSE false
       END AS flag
FROM   src s
       INNER JOIN tgt t

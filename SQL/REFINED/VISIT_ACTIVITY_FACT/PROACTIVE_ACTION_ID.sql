WITH src
     AS (SELECT DISTINCT(VISIT_EVENT_ID) AS VIS_ID,
                ...
                parse_json(VISIT_CUSTOM_VARIABLES):actionId::text as actionId
...
SELECT VIS_ID as src_visitEventid,
       ...
       substr(s.actionid,1,36) AS src_actionid,
       t.PROACTIVE_ACTION_ID AS trg_proactiveActionid,
       CASE
         WHEN src_actionid IS NULL AND trg_proactiveActionid IS NULL THEN true
         WHEN src_actionid = trg_proactiveActionid THEN true
         ELSE false
       END AS flag
FROM   src s
       INNER JOIN tgt t
